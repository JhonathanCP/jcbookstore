...
export const BookStore = {
    ...
    actions: {
        ...
        updateBook: async( {commit, state}, payload ) => { /* state porque se lo mandará al API */
            const {id} = payload
            const res = await BookServices.updateBook(id, state.book)
            commit(UPDATE_BOOK, res.data)
        },
        deleteBook: async( {commit}, payload ) => {
            const res = await BookServices.deleteBook(payload)
    ...
    mutations: {
        ...
        [DELETE_BOOK](state, payload){
            const {books} = state
            const newState = book.filter(book => book._id !== payload)
            state.books = newState
        }
    }
}